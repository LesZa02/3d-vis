...
int main(void)
{
	//íàñòðàèâàåì îáùèå ïàðàìåòðû è íàñòðîéêè
	if (!glfwInit())
	{
	...
	/// íàòðîéñêè äâèæåíèÿ êàìåðû
	//êàìåðà áóäåò äâèãàòüñÿ ïî ãîðèçîíòàëüíîé îêðóæíîñòè ðàäèóñà 7, ñî óãëîâîé ñêîðîñòüþ Pi / 1000 è 
	//ëåëàòü îáîðîòû â âåðòèêàëüíîé ïëîñêîñòè ïî îêðóæíîñòè ðàäèóñà 5.5
	...
	//ïðîíóìåðóåì òî÷êè è âû÷èñëèì èõ öâåòà: íóìåðàöèÿ ïî êðóãó, 
	//ñíà÷àëà âåðõíèå, ïîòîì íèæíèå, îò âíóòðåííèõ ê âíåøíèì
	std::vector<std::vector<GLfloat>> color = color_calc(rect_cnt, face_cnt);
	static GLfloat ring_color[4 * sz];
	//âíóòðÿííÿÿ îáëàñòü
	for (int i = 0; i < rect_cnt; i++) { //íèæíÿÿ ëåâàÿ òî÷êà
		for (int p = 0; p < 4; ++p) {
			ring_color[4 * p_in_rect * i + p] = color[i + 2 * rect_cnt][p];
		}
		for (int p = 0; p < 4; ++p) { //íèæíÿÿ ïðàâàÿ òî÷êà
	...
	static const GLfloat axis_vertex[] = {
		-10.0f, 0.0f, 0.0f, // îñü X
		10.0f, 0.0f, 0.0f,
		0.0f, -10.0f, 0.0f, // îñü Y
		0.0f, 10.0f, 0.0f,
		0.0f, 0.0f, -10.0f,
		0.0f, 0.0f, 10.0f,
	};
	...
	static const GLfloat axis_color[] = {
		1.0f,  1.0f,  1.0f, 0.5f, // îñü X
		1.0f,  1.0f,  1.0f, 0.5f,
		1.0f,  1.0f,  1.0f, 0.5f, // îñü Y
		1.0f,  1.0f,  1.0f, 0.5f,
		1.0f,  1.0f,  1.0f, 0.5f, // îñü Z
		1.0f,  1.0f,  1.0f, 0.5f,
	};
	...
	do {
		...
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); //íå çàáûâàåì î÷èùàòü z-áóôåð
		...
		glBindBuffer(GL_ARRAY_BUFFER, axis_vertex_buf); //êîîðäèíàòû
		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, (void*)0);

		glBindBuffer(GL_ARRAY_BUFFER, axis_color_buf); //öâåò
		glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, 0, (void*)0);
