...
std::vector <std::vector<GLfloat>> color_calc(int rect_cnt, int face_cnt) {
	...
	for (int f = 0; f < face_cnt; ++f) {
		for (int r = 0; r < rect_cnt; ++r) {
			color[f * rect_cnt + r] = { 1.0f * 2 * std::abs(r - rect_cnt / 2) / rect_cnt ,
				0.2f,
				1.0f - 1.0f * 2 * std::abs(r - rect_cnt / 2) / rect_cnt,
				0.5f - 0.25f * (f % (2 * rect_cnt) >= rect_cnt)};
		}
	}
	return color;
}
